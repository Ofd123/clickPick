...
import static com.example.schoolproj.GeminiRelevant.Prompts.EXTRACT_PRODUCTS_PROMPT;
 ...
 * Allows users to define search parameters (attributes/settings), performs a
 * web search via Exa,
...
public class search_screen extends MasterActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (json != null)
            parseInitialSearchDetails(json);
        else {
    ...
    /**
     * Parses initial search details from a JSON string (typically from image
     * analysis).
     * 
    ...
    private void parseGeminiResponse(String response) {
        try {
            ...
            for (int i = 0; i < products.length(); i++) {
                ...
                if (priceType == null)
                    priceType = "USD"; // Default or logic to extract

                results.add(new Product(name, price, priceType, image, description, storeName, url, storeLocation,
                        otherDetails));
            }
        } catch (Exception e) {
    ...
    /**
     * Safely retrieves a string from a JSONObject, handling nulls and trimming
     * values.
     * 
    ...
    public void startSearch(View view) {
        ...
        for (SearchItemParameter p : searchParameters) {
            ...
            if (setting != null && !setting.isEmpty() && !setting.equalsIgnoreCase("null")
                    && !setting.equalsIgnoreCase("undefined") && !setting.equalsIgnoreCase("any")
                    && !setting.equalsIgnoreCase("anything")) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    String msg = EXTRACT_PRODUCTS_PROMPT + text.toString();
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    if (result == null || result.trim().isEmpty()) {
                                        Toast.makeText(search_screen.this, "Empty response from AI", Toast.LENGTH_SHORT)
                                                .show();
                                        return;
                                    }
                                    if (result.contains("\"error\":503")) {
                                        Toast.makeText(search_screen.this,
                                                "AI is currently busy (server overloaded). Try again shortly.",
                        ...
                        public void onFailure(Throwable error) {
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    {
                                        Toast.makeText(search_screen.this, "Gemini failed: " + error.getMessage(),
                                                Toast.LENGTH_LONG).show();
                                    }
                                }
                            });
    ...
    /**
     * Finalizes the search by saving the results to history in Firebase and
    ...
    /**
     * Updates the visibility of "Add" buttons based on whether the last parameter
     * is correctly filled.
     * Enforces a consistency rule where parameters must be completed before adding
     * new ones.
     */
    private void updateAddButtonVisibility() {
        ...
        // If it's the first item, we only care about the setting (product name is
        // fixed)
        ...
        if (btnCard != null)
            btnCard.setVisibility(visibility);
        if (btnBottom != null)
